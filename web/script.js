...
function filterByGenre(genre) {
  ...
  if (movies.length === 0) {
    slider.innerHTML =
      "<p style='color:#777; padding:20px;'>Belum ada film di genre ini.</p>";
  } else {
...
function searchMovies(query) {
  ...
  if (matchedMovies.length === 0) {
    slider.innerHTML =
      "<p style='color:#777; padding:20px;'>Tidak ditemukan film.</p>";
  } else {
...
async function saveEdit() {
  ...
  if (res.status === "success") {
    ...
    activeMovie.title = title;
    activeMovie.genre = genre;
    activeMovie.synopsis = synopsis;
...
async function doUpload() {
  ...
  let result = await eel.process_upload(
    title,
    genre,
    synopsis,
    vPath,
...
function toggleSearchBox() {
  ...
  // Otomatis fokus ke input agar ketikan langsung muncul
